  ...
  try {
    ...
    if (response.ok) {
      await db.collection("emails").insertOne({
        ...
        subject,
        html,
        dedupKey, // On stocke la clé pour le prochain check
        sentAt: new Date(),
        status: 'success',
        source: 'sync-engine'
      });
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // --- PURGE DES VIEILLES MISSIONS (2 MOIS+) ---
        ...
        if (dtEndMatch && dtEndMatch[1]) {
          ...
          if (formattedDate >= todayStr) {
            ...
            if (!existing) {
              ...
              // NOTIFICATION NOUVEAUTÉ AVEC DEDUP
              ...
              for (const agent of eligibleAgents) {
                if (agent.email) {
                  ...
                  await sendEmailWithDedup(
                    agent.email, 
                    `[NOUVEAU] Mission : ${prop.id.toUpperCase()} (${formattedDate})`,
                    `<p>Bonjour ${agent.name}, une mission est disponible pour ${prop.id.toUpperCase()} le ${formattedDate}.</p>`,
                    dedupKey,
                    db
                  );
                }
    ...
    // --- 2. NETTOYAGE DES ANNULATIONS ---
    const futureMissions = await missionsCol.find({ calendarEventId: { $exists: true }, date: { $gte: todayStr } }).toArray();
    for (const m of futureMissions) {
      ...
      for (const m of allMissions) {
        ...
        if (m.date === todayStr && m.status === 'assigned' && m.cleanerId) {
          ...
          if (cleaner?.email) {
            ...
            await sendEmailWithDedup(
              cleaner.email,
              `[RAPPEL] Mission aujourd'hui : ${m.propertyId.toUpperCase()}`,
              `<p>Bonjour ${cleaner.name}, rappel de votre mission aujourd'hui.</p>`,
              dedupKey,
              db
            );
            report.reminders++;
        ...
        if (m.date === in7DaysStr && m.status === 'pending') {
          ...
          for (const a of eligible) {
            if (a.email) {
              ...
              await sendEmailWithDedup(
                a.email,
                `[URGENT J-7] Mission toujours libre : ${m.propertyId.toUpperCase()}`,
                `<p>La mission du ${m.date} n'est toujours pas assignée.</p>`,
                dedupKey,
                db
              );
            }
